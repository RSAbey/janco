...
const EditTransactionForm = ({ isOpen, onClose, onSave, transaction }) => {
  ...
  const [currentPaymentSlip, setCurrentPaymentSlip] = useState("")
  ...
  useEffect(() => {
    if (transaction && isOpen) {
      ...
      setCurrentPaymentSlip(transaction.paymentSlip || "")
      setPaymentSlip(null)
      setError("")
    }
  }, [transaction, isOpen])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let paymentSlipUrl = currentPaymentSlip
  ...
  const handleRemovePaymentSlip = () => {
    setCurrentPaymentSlip("")
    setPaymentSlip(null)
  }
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            {currentPaymentSlip && !paymentSlip && (
              <div className="mb-2 p-2 bg-gray-50 rounded border">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Current payment slip attached</span>
                  <button
                    type="button"
                    onClick={handleRemovePaymentSlip}
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-4">
              <input
                ...
                id="editPaymentSlip"
              />
              <label htmlFor="editPaymentSlip" className="flex flex-col items-center cursor-pointer">
                <FaUpload className="text-gray-400 text-2xl mb-2" />
                <span className="text-sm text-gray-600">
                  {paymentSlip ? paymentSlip.name : "Click to upload new payment slip"}
                </span>
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            >
              {loading ? (uploading ? "Uploading..." : "Updating...") : "Update Transaction"}
            </button>
          </div>
        </form>
      </div>
